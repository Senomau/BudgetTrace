...
BudgetTrace 是面向生产系统的工具型长程 Agent 运行时预算控制与审计层。它位于 Agent、模型网关和工具执行器外侧，记录每一步轨迹，核算 token、时间和成本预算，并在预算窗口选择继续、重规划、切换低成本模型或停止。
...
上面命令不调用模型端点。`output/` 是本地生成目录，已加入 `.gitignore`，不要提交。
...
不要把 key 写进文件、报告、截图或 issue。live 输出是 informational，`success` 默认只表示模型发出了结束动作，不是 benchmark 分数；使用 `reference-actions` scorer 时，结果只表示声明动作契约是否满足。
...
这是一个可独立运行的 Agent Runtime / Evaluation Infrastructure 项目，核心交付是“预算约束下的长程工具 Agent 执行闭环”：统一事件、账本、工具安全、控制决策、评分和 replay，并用固定任务矩阵验证成功率、晚突破误停率和单位成功成本。

当前公开证据包括：181 项自动化测试；5 任务 × 3 trial × 2 fixture route × 4 策略的 120-run 矩阵；τ³-bench v1.0.1 的 10 任务审计与 37 个参考动作回放；DeepSeek、GLM、MiniMax 等 OpenAI-compatible 端点的真实多轮管线记录。`reference-actions` 是声明动作契约代理，真实业务状态/数据库 scorer 尚未接入，因此简历中应写“业务动作契约可验证”，不要写成线上交易成功率或生产收益。
