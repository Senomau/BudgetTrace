...
Copy `data/public/tau3-retail-v1/model-routes.example.json`, keep only public endpoint/model/price metadata in the file, and set each route's `api_key_env` variable in the current process. Never put a key in JSON, reports, journals, screenshots, or shell scripts.
...
## Practical and resume positioning

This release is strong evidence for an Agent Runtime, evaluation infrastructure, or LLM systems role: it contains a complete auditable loop, explicit safety and budget invariants, deterministic replay, multi-trial accounting, and real-provider adapter tests. It does not claim RL training, a production deployment, or a real business database scorer. Those are separate follow-up work items: connect a versioned business oracle, persist and resume events, add idempotency and compensation for side effects, and run service-level load and fault tests.

See [the runbook](docs/runbook.md), [release verification](docs/release-verification.md), and [security policy](SECURITY.md) for reproducible commands and release boundaries.
